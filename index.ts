...
const VALID_OUTPUT_FORMATS = ["jpeg", "png"] as const;
type OutputFormat = typeof VALID_OUTPUT_FORMATS[number];

// Valid acceleration levels
const VALID_ACCELERATION_LEVELS = ["none", "regular", "high"] as const;
type AccelerationLevel = typeof VALID_ACCELERATION_LEVELS[number];
    ...
    case "generate_image": {
      try {
        ...
        try {
          ...
          const generationPromise = fal.subscribe("fal-ai/qwen-image", {
            ...
            onQueueUpdate: (update: any) => {
              if (update.status === "IN_PROGRESS") {
                update.logs?.map((logEntry: any) => logEntry.message).forEach((message: string) => {
                  log('debug', `Generation progress: ${message}`);
                });
              }
            },
          }) as Promise<{ data: FalImageResponse; requestId: string }>;
          
          const result = await Promise.race([generationPromise, timeoutPromise]) as { data: FalImageResponse; requestId: string };
          ...
          const downloadedImages: { url: string; localPath: string; index: number; width: number; height: number }[] = [];
